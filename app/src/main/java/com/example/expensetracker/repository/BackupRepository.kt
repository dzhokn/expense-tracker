...
class BackupRepository(
    ...
    suspend fun performBackup(backupFolderUri: Uri): BackupResult = withContext(Dispatchers.IO) {
        try {
            ...
            // Find existing backup file or stale copies (SAF may have appended suffixes)
            ...
            context.contentResolver.query(
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (name == BACKUP_FILENAME) {
                        // Reuse exact-match file
                        existingDocUri = DocumentsContract.buildDocumentUriUsingTree(backupFolderUri, docId)
                    } else if (name.startsWith("expense_tracker_backup") && name.endsWith(".zip")) {
                        // Delete stale copies with appended suffixes like " (1)"
                        val staleUri = DocumentsContract.buildDocumentUriUsingTree(backupFolderUri, docId)
                        try { DocumentsContract.deleteDocument(context.contentResolver, staleUri) } catch (_: Exception) {}
                    }
                }
            }

            // Overwrite existing file, or create new one if none found
            val docUri = existingDocUri ?: DocumentsContract.createDocument(
            ...
            // "wt" = write + truncate — overwrites existing content completely
            context.contentResolver.openOutputStream(docUri, "wt")?.use { outputStream ->
