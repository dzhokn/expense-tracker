        ...
        Box(
            ...
            Canvas(
                    ...
                    .pointerInput(data) {
                        detectTapGestures { offset ->
                            ...
                            for (i in data.indices) {
                                ...
                                val barFraction =
                                    data[i].totalAmount.toFloat() / maxAmount
                                val barHeight = chartHeight * barFraction
                                val barY = chartHeight - barHeight

                                if (offset.x in barX..(barX + barWidth) &&
                                    offset.y in 0f..chartHeight
                                ) {
                                    selectedBarIndex =
                                        if (selectedBarIndex == i) -1 else i
                ...
                if (selectedBarIndex in data.indices) {
                    val selected = data[selectedBarIndex]
                    val yearMonth = try {
                        YearMonth.parse(selected.yearMonth)
                    } catch (e: Exception) {
                        null
                    }
                    ...
                    val tooltipText =
                        "$monthName: ${CurrencyFormatter.format(selected.totalAmount)}"
                    ...
                    val measuredTooltip =
                        textMeasurer.measure(tooltipText, tooltipStyle)
                    val tooltipPadding = 8.dp.toPx()
                    val tooltipWidth =
                        measuredTooltip.size.width + tooltipPadding * 2
                    val tooltipHeight =
                        measuredTooltip.size.height + tooltipPadding * 2

                    val barX = spacing + selectedBarIndex * (barWidth + spacing)
                    val barFraction =
                        selected.totalAmount.toFloat() / maxAmount
                    val barHeight = chartHeight * barFraction
                    val barY = chartHeight - barHeight

                    var tooltipX =
                        barX + barWidth / 2 - tooltipWidth / 2
                    tooltipX =
                        tooltipX.coerceIn(0f, size.width - tooltipWidth)
                    val tooltipY = (barY - tooltipHeight - 4.dp.toPx())
                        .coerceAtLeast(0f)
