        ...
        when {
            ...
            uiState.selectedUri != null -> {
                FilePreviewState(
                    ...
                    onChangeFile = { filePicker.launch(arrayOf("text/*")) }
                )
            }
            // State 1: No file selected
            else -> {
                FileSelectionState(
                    onSelectFile = { filePicker.launch(arrayOf("text/*")) }
                )
            }
        }
    }
...
private fun FileSelectionState(onSelectFile: () -> Unit) {
    Column(
        ...
        Button(
            ...
            Text("Select CSV File", color = DarkBackground)
        }
    }
}
